...
///////////////// rserv 0.1 /// a simple REST prototyping server ////////////
-----------------------------------------------------------------------------
    """
    print(notice)
...
def delete_entity(entity, id):
    ...
    if os.path.exists(file_path):
        ...
        return jsonify({"message": f"{entity} with id {id} deleted successfully"}), 200
    else:
...
if __name__ == '__main__':
    ...
    logger.info(f"Starting rserv 0.1 on {config['host']}:{config['port']}")
